...
#     label_maps = defaultdict(list)
#     for i, label in enumerate(dataset.data.y.t().squeeze().tolist()):
#         label_maps[label].append(i)
#     return label_maps
...
def get_subset_list(dataset, index_list, dropedge=0):
    ...
    if dropedge > 0:
        for data in data_list:
            data.edge_index, data.edge_attr = dropout_adj(data.edge_index, data.edge_attr, p=dropedge)
    return data_list
...
def get_dataloaders(dataset, split_idx, holdout, args, dropedge):
    ...
    dataset_p, dataset_q = dataset[split_idx["train"]], dataset[split_idx["test"]]
    
    dataset_p_list = [g for g in dataset_p]
    dataset_q_list = [g for g in dataset_q]
    subset_p_index, subset_q_index = create_label_shift_free_datasets([dataset_p_list, dataset_q_list], seed)
