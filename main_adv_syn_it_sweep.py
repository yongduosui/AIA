...
def plot_test(test_list_iid, test_list_ood, file_name):

    x = [i for i in range(len(test_list_iid))]
...
def main(args):
    ...
    hyperparameter_defaults = {   
        ...
        "adv_dis": args.adv_dis,
        "adv_reg": args.adv_reg,
        "cau_reg": args.cau_reg,
        "attacker_lr": args.attacker_lr,
        "causaler_lr": args.causaler_lr
    }

    wandb.init(config=hyperparameter_defaults, project="good-motif")
    config = wandb.config

    file_name = "motif_baseline"
    eval_metric = "rocauc"
    data_dir = "dataset"
    device = torch.device("cuda:" + str(args.device)) if torch.cuda.is_available() else torch.device("cpu")

    if args.dataset == "cmnist":
        dataset, meta_info = GOODCMNIST.load(data_dir, domain=args.domain, shift='covariate', generate=False)
        num_class, num_layer, in_dim = 10, 3, 3
    elif args.dataset == "motif":
        dataset, meta_info = GOODMotif.load(data_dir, domain=args.domain, shift='covariate', generate=False)
        num_class, num_layer, in_dim = 3, 3, 1
    else:
        assert False
    batch_size = config.batch_size
    train_loader     = DataLoader(dataset["train"],  batch_size=batch_size, shuffle=True)
    valid_loader_ood = DataLoader(dataset["val"],    batch_size=batch_size, shuffle=False)
    valid_loader_iid = DataLoader(dataset["id_val"], batch_size=batch_size, shuffle=False)
    test_loader_ood  = DataLoader(dataset["test"],   batch_size=batch_size, shuffle=False)
    test_loader_iid  = DataLoader(dataset["id_test"],batch_size=batch_size, shuffle=False)

    model = CausalAdvGNNSyn(num_class=num_class, 
                            in_dim=in_dim,
                            emb_dim=300,
                            fro_layer=config.fro_layer,
                            bac_layer=config.bac_layer,
                            cau_layer=config.cau_layer,
                            att_layer=config.att_layer, 
                            cau_gamma=config.cau_gamma,
                            adv_gamma=config.adv_gamma).to(device)
    
    opt_attacker = optim.Adam(model.attacker.parameters(), lr=config.attacker_lr)
    ...
    for epoch in range(config.epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            loss_causaler = loss_cau + loss_com + loss_reg * config.cau_reg
            ...
            loss_attacker = loss_adv - loss_dis * config.adv_dis - loss_reg * config.adv_reg
        ...
        metrics = {'valid_result_ood': valid_result_ood * 100,
                   'test_result_iid ': test_result_iid * 100,
                   'test_result_ood' : test_result_ood * 100, 
                   'epoch_loss_causaler': epoch_loss_causaler,
                   'epoch_loss_attacker': epoch_loss_attacker,
                   'highest_valid_ood'   : results['highest_valid_ood'] * 100,
                   'update_test_ood': results['update_test_ood'] * 100}
        wandb.log(metrics)
...
if __name__ == "__main__":
    ...
    def arg_parse():
        ...
        parser.add_argument('--domain', type=str, default='basis', help='color, background')
